...
function parseSupportedSourceReference(classification) {
  ...
  return parseSkillsIlCommand(reference) || parseNpmCommand(reference) || parsePypiCommand(reference) || parseNpmReference(reference) || parsePypiUrl(reference) || parseGithubUrl(reference);
}
...
async function fetchBuffer(url, allowedHost, maxBytes) {
  ...
  if (parsed.protocol !== "https:" || parsed.hostname !== allowedHost) throw new Error("The package artifact host is not trusted.");
  const response = await fetchWithTimeout(url);
  if (!response.ok) throw new Error(`Package artifact download failed (${response.status}).`);
  if (new URL(response.url || url).hostname !== allowedHost) throw new Error("The package artifact redirected to an untrusted host.");
...
module.exports = { parseSkillsIlCommand, parseNpmCommand, parseNpmReference, parsePypiCommand, parsePypiUrl, parseGithubUrl, parseSupportedSourceReference, resolveSupportedSource };
