...
function sourcePathRank(path) {
  ...
  if (/(install|postinstall|setup|hook|auth|credential|secret|token|env|config)/.test(value)) score += 12;
  ...
  if (/\.(sh|bash|ps1|bat|cmd)$/.test(value)) score += 6;
  if (/\.(md|txt)$/.test(value)) score -= 4;
  return score;
}

// Pick the highest-risk files that fit the deep-scan budget instead of refusing large sources.
// Coverage is reported so the scan verdict can be capped at "needs review" when incomplete.
function selectRankedTextFiles(candidates, skippedLargeCount = 0) {
  const ranked = [...candidates].sort((a, b) => sourcePathRank(b.path) - sourcePathRank(a.path) || a.text.length - b.text.length);
  const files = [];
  let totalChars = 0;
  for (const file of ranked) {
    if (files.length >= MAX_SOURCE_FILES) break;
    if (totalChars + file.text.length > MAX_SOURCE_CHARS) continue;
    files.push(file);
    totalChars += file.text.length;
  }
  const filesTotal = candidates.length + skippedLargeCount;
  return { files, coverage: { complete: files.length === filesTotal, files_scanned: files.length, files_total: filesTotal } };
}
...
function extractTarTextFiles(bytes, prefixToStrip = "") {
  ...
  while (offset + 512 <= bytes.length) {
    ...
    if ((type === "" || type === "0" || type === "\0") && usefulSourcePath(cleanPath)) {
      ...
      if (size > MAX_FILE_CHARS) skippedLarge += 1;
      else candidates.push({ path: cleanPath, text: bytes.subarray(start, end).toString("utf8") });
    }
    offset = start + Math.ceil(size / 512) * 512;
  }
  if (!candidates.length) throw new Error("No supported source files were found in the package artifact.");
  return selectRankedTextFiles(candidates, skippedLarge);
}
...
function extractZipTextFiles(bytes, prefixToStrip = "") {
  ...
  // First pass: list every relevant entry so files can be risk-ranked before extraction
  const entries = [];
  let skippedLarge = 0;
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (uncompressedSize > MAX_FILE_CHARS) { skippedLarge += 1; continue; }
    entries.push({ path, method, compressedSize, uncompressedSize, localOffset });
  }
  if (!entries.length) throw new Error("No supported source files were found in the extension package.");

  // Second pass: extract the highest-risk entries that fit the deep-scan budget
  entries.sort((a, b) => sourcePathRank(b.path) - sourcePathRank(a.path) || a.uncompressedSize - b.uncompressedSize);
  const files = [];
  let totalChars = 0;
  for (const entry of entries) {
    if (files.length >= MAX_SOURCE_FILES) break;
    if (totalChars + entry.uncompressedSize > MAX_SOURCE_CHARS) continue;
    if (entry.localOffset + 30 > bytes.length || bytes.readUInt32LE(entry.localOffset) !== 0x04034b50) throw new Error("The extension archive entry is malformed.");
    ...
    else if (entry.method === 8) content = zlib.inflateRawSync(compressed, { maxOutputLength: MAX_FILE_CHARS + 1 });
    else throw new Error("The extension archive uses an unsupported compression method.");
    if (content.length !== entry.uncompressedSize || content.length > MAX_FILE_CHARS) throw new Error("The extension archive failed its size verification.");
    files.push({ path: entry.path, text: content.toString("utf8") });
    totalChars += content.length;
  }
  const filesTotal = entries.length + skippedLarge;
  return { files, coverage: { complete: files.length === filesTotal, files_scanned: files.length, files_total: filesTotal } };
}
...
async function resolveOpenVsxExtension(source) {
  ...
  const { files, coverage } = extractZipTextFiles(bytes, "extension/");
  const scannedVersion = extensionVersion(files, version);
  return { code: joinedSource(files), provider: "open_vsx", source_url: `https://open-vsx.org/extension/${source.publisher}/${source.extension}/${scannedVersion}`, source_owner: source.publisher, source_name: `${source.publisher}.${source.extension}@${scannedVersion}`, source_ref: scannedVersion, files: files.map(file => file.path), coverage };
}
...
async function resolveVsMarketplaceExtension(source) {
  ...
  const { files, coverage } = extractZipTextFiles(bytes, "extension/");
  const scannedVersion = extensionVersion(files, version);
  return { code: joinedSource(files), provider: "vs_marketplace", source_url: source.source_url, source_owner: source.publisher, source_name: `${source.publisher}.${source.extension}@${scannedVersion}`, source_ref: scannedVersion, files: files.map(file => file.path), coverage };
}
...
async function resolveNpmPackage(source) {
  ...
  const { files, coverage } = extractTarTextFiles(zlib.gunzipSync(bytes), "package/");
  return { code: joinedSource(files), provider: "npm", source_url: `${source.source_url}/v/${version}`, source_owner: String(release.author?.name || ""), source_name: `${source.package_name}@${version}`, source_ref: version, files: files.map(file => file.path), coverage };
}
...
async function resolvePypiPackage(source) {
  ...
  const { files, coverage } = extractTarTextFiles(zlib.gunzipSync(bytes));
  return { code: joinedSource(files), provider: "pypi", source_url: `https://pypi.org/project/${source.package_name}/${version}/`, source_owner: String(metadata?.info?.author || ""), source_name: `${source.package_name}==${version}`, source_ref: version, files: files.map(file => file.path), coverage };
}
...
async function resolveSupportedSource(classification) {
  ...
  // GitHub truncates the listing for very large repos — scan proceeds on the highest-risk
  // files and coverage is reported so the verdict gets capped at "needs review", never "safe"
  const listingComplete = !tree.truncated;
  ...
  const candidates = scopedBlobs.filter(item => usefulSourcePath(item.path));
  if (!candidates.length) throw new Error("No supported source files were found at the referenced source.");
  ...
  for (const file of files) {
    if (included.length >= MAX_SOURCE_FILES) break;
    if (code.length + (file.size || 0) > MAX_SOURCE_CHARS) continue;
    const text = await fetchText(`https://raw.githubusercontent.com/${source.owner}/${source.repo}/${ref}/${file.path}`);
    const block = `\n\n// Source file: ${file.path}\n${text}`;
    if (code.length + block.length > MAX_SOURCE_CHARS) continue;
    code += block;
    included.push(file.path);
  }
  if (!code.trim()) throw new Error("The referenced source could not be downloaded safely.");
  const coverage = {
    complete: listingComplete && included.length === candidates.length,
    files_scanned: included.length,
    files_total: candidates.length,
  };
  return { code: code.trim(), provider: source.provider, source_url: source.source_url, source_owner: source.owner, source_name: `${source.owner}/${source.repo}${source.path ? `/${source.path}` : ""}`, source_ref: ref, files: included, coverage };
}
