...
// "Verified" = both independent automated layers (AI + deterministic static rules)
// agree the code is clean with a high security score. A dynamic sandbox is not
// part of the live product, so it is not required; when one runs and confirms
// clean, sandboxRanClean() can be used to mark an even higher assurance tier.
function isVerifiedInstallResult(result, decisionOverride = "") {
  return cleanAutomatedReview(result, decisionOverride);
}

function verificationLevelForResult(result, decisionOverride = "") {
  return cleanAutomatedReview(result, decisionOverride) ? "verified" : "";
}
