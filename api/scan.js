...
const STATIC_RULES = [
  {
    ...
    severity: "CRITICAL",
    score: 95,
    pattern: /(bash\s+-i\s*>\s*&\s*\/dev\/tcp|\bnc\s+-e\s+|\bncat\s+-e\s+|socket\.connect\s*\()/i,
  ...
  },
  {
    ...
    severity: "HIGH",
    score: 80,
    pattern: /(process\.env|os\.environ|\bgetenv)\s*(\.|\[|\()\s*['"]?(OPENAI_API_KEY|ANTHROPIC_API_KEY|GITHUB_TOKEN|AWS_SECRET|AWS_ACCESS_KEY|DATABASE_URL|PRIVATE_KEY|SECRET_KEY|PASSWORD|AUTH_TOKEN)/i,
  ...
  },
  {
    ...
    severity: "HIGH",
    score: 75,
    pattern: /(\bchild_process|\bexecSync\s*\(|\bspawn\s*\(|subprocess\.(run|Popen|call|check_output)|os\.system\s*\()/i,
  ...
  },
  {
    ...
    severity: "MEDIUM",
    score: 55,
    pattern: /\b(pip|pip3|npm|pnpm|yarn)\s+install.{0,120}(https?:\/\/|git\+|github\.com|bitbucket\.org|gitlab\.com)/i,
...
const SUPPLEMENTAL_STATIC_RULES = [
  ...
  { family: "CODE_INJECTION", severity: "HIGH", score: 75, pattern: /(new Function|\bcompile\s*\(|vm\.runIn|\bexec\s*\(|\beval\s*\().{0,120}(input|request|query|params|\buser)/i, description: THREAT_FAMILY_DEFINITIONS.CODE_INJECTION },
  { family: "SHELL_ESCAPE", severity: "HIGH", score: 75, pattern: /\b(exec|execSync|execFile|spawn|spawnSync|popen|os\.system|subprocess\.(?:run|call|Popen|check_output))\s*\(\s*[^)]{0,200}(\$\(|\$\{|&&|\|\||;|\s\|\s)/i, description: THREAT_FAMILY_DEFINITIONS.SHELL_ESCAPE },
  { family: "SQL_INJECTION", severity: "HIGH", score: 70, pattern: /\b(select|insert|update|delete)\b.{0,80}(\+|`|\$\{|format\(|%s).{0,120}(req\.|request|query|params|input|\buser)/i, description: THREAT_FAMILY_DEFINITIONS.SQL_INJECTION },
  ...
  { family: "TEMPLATE_INJECTION", severity: "HIGH", score: 70, pattern: /(render_template_string|template\.render|\bjinja|\bhandlebars|\bmustache|ejs\.render).{0,120}(\breq\b|request|query|params|input|\buser)/i, description: THREAT_FAMILY_DEFINITIONS.TEMPLATE_INJECTION },
  ...
  { family: "ENV_VAR_THEFT", severity: "HIGH", score: 80, pattern: /(process\.env|os\.environ|\benviron\b).{0,80}(forEach|Object\.keys|\bdump|\bprint|\bsend\b|\bpost\b|\bupload)/i, description: THREAT_FAMILY_DEFINITIONS.ENV_VAR_THEFT },
  { family: "NETWORK_CALLBACK", severity: "MEDIUM", score: 55, pattern: /\b(fetch|axios|requests|httpx|curl|wget)\b.{0,120}(https?:\/\/|ngrok|webhook|requestbin|pipedream|interact\.sh)/i, description: THREAT_FAMILY_DEFINITIONS.NETWORK_CALLBACK },
  ...
  { family: "DATA_HARVESTING", severity: "HIGH", score: 70, pattern: /\b(glob|walk|readdir|find)\b\s*.{0,120}(\.ssh|\.aws|Documents|Desktop|\bsource|\brepo|password|token|secret)/i, description: THREAT_FAMILY_DEFINITIONS.DATA_HARVESTING },
  { family: "CREDENTIAL_THEFT", severity: "CRITICAL", score: 90, pattern: /(\.ssh\/id_rsa|\.netrc|keychain|Credential Manager|Login Data|\bpasswords?\b|\bcredentials?\b|\bcookies?\b).{0,120}(\bread\b|\bcopy\b|\bsend\b|\bpost\b|\bupload\b|\bexfiltrat)/i, description: THREAT_FAMILY_DEFINITIONS.CREDENTIAL_THEFT },
  ...
  { family: "CHAR_CODE_OBFUSCATION", severity: "MEDIUM", score: 45, pattern: /(String\.fromCharCode|\bchr\s*\(|charCodeAt).{0,120}(\beval\b|\bexec\b|\bFunction\b|\bjoin\b)/i, description: THREAT_FAMILY_DEFINITIONS.CHAR_CODE_OBFUSCATION },
  ...
  { family: "ZIP_BOMB", severity: "HIGH", score: 70, pattern: /\b(zipfile|tarfile|adm-zip|extractAll|unzip)\b.{0,120}(\brecursive|\bwhile\b|forEach|\bextract)/i, description: THREAT_FAMILY_DEFINITIONS.ZIP_BOMB },
  ...
  { family: "BIND_SHELL", severity: "CRITICAL", score: 90, pattern: /(\bnc\s+-l|\bnetcat\s+-l|server\.listen\s*\(|socket\.bind\s*\().{0,120}(\bsh\b|\bbash\b|\bcmd\b|powershell|\bexec)/i, description: THREAT_FAMILY_DEFINITIONS.BIND_SHELL },
  { family: "C2_CALLBACK", severity: "CRITICAL", score: 90, pattern: /(command.?and.?control|\bc2\b|\bbeacon\b|\bimplant\b|\bpayload\b).{0,120}(\bhttps?\b|\bsocket\b|\bpoll\b|\bcallback\b)/i, description: THREAT_FAMILY_DEFINITIONS.C2_CALLBACK },
  ...
  { family: "SUDO_ABUSE", severity: "HIGH", score: 75, pattern: /(\bsudo\s+|\bsudoers|\bsudo -S|echo\s+.*\|\s*sudo)/i, description: THREAT_FAMILY_DEFINITIONS.SUDO_ABUSE },
  ...
  { family: "SYMLINK_ATTACK", severity: "MEDIUM", score: 55, pattern: /(\bsymlink|\bln\s+-s|fs\.symlink|os\.symlink).{0,120}(\/etc|\.ssh|\.aws|\broot\b|\bhome\b)/i, description: THREAT_FAMILY_DEFINITIONS.SYMLINK_ATTACK },
  { family: "CRYPTO_MINING", severity: "CRITICAL", score: 90, pattern: /\b(xmrig|stratum\+tcp|monero|cryptonight|nicehash|minerd|coinhive)\b/i, description: THREAT_FAMILY_DEFINITIONS.CRYPTO_MINING },
  { family: "RANSOMWARE_PATTERN", severity: "CRITICAL", score: 95, pattern: /(\bransom|decrypt key|encryptFiles|crypto\.createCipher|\bFernet\b).{0,160}(readdir|\bwalk\b|\bglob\b|\bfiles\b|extension)/i, description: THREAT_FAMILY_DEFINITIONS.RANSOMWARE_PATTERN },
  ...
  { family: "DEPENDENCY_CONFUSION", severity: "MEDIUM", score: 55, pattern: /(registry\.npmjs\.org|pypi\.org|pip install|npm install).{0,120}(\binternal\b|\bprivate\b|\bcorp\b|\bcompany\b|@[^\/]+\/)/i, description: THREAT_FAMILY_DEFINITIONS.DEPENDENCY_CONFUSION },
  { family: "TYPOSQUATTING", severity: "LOW", score: 35, pattern: /\b(reqeusts|requsts|expres[^s]|lodahs|reactt|djagno|flsak|openaii|anthropc|githb)/i, description: THREAT_FAMILY_DEFINITIONS.TYPOSQUATTING },
  { family: "TIME_BASED_ATTACK", severity: "MEDIUM", score: 45, pattern: /(setTimeout|setInterval|\bsleep\b|\bcron\b|Date\.now|datetime\.now).{0,120}(\bpayload|\bexec|\bdelete|\bencrypt|\bdownload|\bshell)/i, description: THREAT_FAMILY_DEFINITIONS.TIME_BASED_ATTACK },
  { family: "LOGIC_BOMB", severity: "HIGH", score: 70, pattern: /(\bif\s*\(.{0,80}(hostname|username|\bdate\b|\benv\b|process\.env).{0,160}(\bdelete|\bencrypt|\bexec|\bpayload|\bshell))/i, description: THREAT_FAMILY_DEFINITIONS.LOGIC_BOMB },
  ...
  { family: "COOKIE_THEFT", severity: "HIGH", score: 80, pattern: /(document\.cookie|chrome\.cookies|localStorage|sessionStorage).{0,120}(\bsend\b|\bpost\b|\bfetch\b|\bupload|\bexfiltrat|\btoken)/i, description: THREAT_FAMILY_DEFINITIONS.COOKIE_THEFT },
  ...
  { family: "FILE_SYSTEM_ATTACK", severity: "HIGH", score: 80, pattern: /(\brsync|\brobocopy|\bxcopy|copy-item|\bcp\s+|\bmv\s+|shutil\.(copy|move)|fs\.(copyFile|rename)).{0,220}(\.env|\.ssh|\.aws|credentials|secrets?|tokens?|keychain|Login Data).{0,220}(\/tmp|tmpdir|public|shared|uploads?|cache|clipboard|Downloads)/is, description: "Living-off-the-land file staging: standard copy/sync tools move sensitive files toward a less protected location." },
  { family: "DATA_HARVESTING", severity: "HIGH", score: 80, pattern: /(\.env|process\.env|os\.environ|\.ssh|\.aws|credentials|tokens?|secrets?).{0,500}(\bfetch\b|\baxios|requests\.(post|put)|httpx\.(post|put)|\bcurl\b|webhook|\bupload|\bsend\b|\bexfiltrat)/is, description: "Non-adjacent data flow: sensitive data appears to move from a local source toward an external sink." },
  { family: "LOGIC_BOMB", severity: "HIGH", score: 75, pattern: /\b(if|switch|case|when)\b.{0,220}(crypto|wallet|\bseed\b|mnemonic|private[_ -]?key|backup|invoice|payroll|production|deploy).{0,260}(\bexec|\bspawn|subprocess|\bfetch\b|requests|axios|\bdelete|\bencrypt|\bupload|\bsend\b)/is, description: "Input-dependent activation: risky behavior is gated behind a rare user request or sensitive keyword." },
  ...
  { family: "SUPPLY_CHAIN_ATTACK", severity: "HIGH", score: 75, pattern: /\b(postinstall|preinstall|prepare|install)\b.{0,260}(node-gyp|\.node|\bdll\b|\.so|\.dylib|\bcurl\b|\bwget\b|powershell|\bbash\b|\bpython).{0,260}(\bdownload|\bhttp|\btmp\b|\bchmod|\bexec|\bspawn)/is, description: "Install-time dynamic payload or native module behavior can run before the user reviews the package." },
];
